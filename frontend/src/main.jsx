...
import {Activity,Bus,MapPin,ShieldAlert,Waves,Gauge,HeartPulse,Route,Wrench} from 'lucide-react';
...
function GISMap({geojson}){
  ...
  useEffect(()=>{
    ...
    const update=()=>{const src=map.getSource('issues');if(src)src.setData(geojson)};
...
function App(){
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [s,i,h,g,w]=await Promise.all([
          fetch(`${API}/api/v1/stats`),fetch(`${API}/api/v1/issues`),fetch(`${API}/api/v1/analytics/urban-health`),fetch(`${API}/api/v1/gis/issues.geojson`),fetch(`${API}/api/v1/authority/work-items`)
        ]);
        if(s.ok)setStats(await s.json());
        if(i.ok)setIssues(await i.json());
        if(h.ok)setHealth(await h.json());
        if(g.ok)setGeojson(await g.json());
        if(w.ok)setWorkItems(await w.json());
        setOnline(s.ok&&i.ok&&h.ok&&g.ok);
  ...
  return <main>
    ...
    <section className="stats">
      <Stat icon={Activity} label="Events this run" value={stats.total_events}/><Stat icon={MapPin} label="Correlated issues" value={stats.correlated_issues}/><Stat icon={Bus} label="Active buses" value={stats.active_bus_ids.length}/><Stat icon={Wrench} label="Authority jobs" value={stats.authority_work_items||0}/>
    </section>
    <section className="healthGrid">
      <Stat icon={HeartPulse} label="Road health" value={health.road_health_score} suffix="%"/><Stat icon={Gauge} label="Congestion index" value={health.congestion_index} suffix="%"/><Stat icon={ShieldAlert} label="Safety risk" value={health.safety_risk_score} suffix="%"/><Stat icon={Route} label="Persisted events" value={stats.persisted_events||0}/>
    </section>
    <section className="grid">
      ...
      <div className="panel"><h2>Authority Workflow</h2><div className="feed">{workItems.length===0?<p className="muted">No authority work items yet.</p>:workItems.slice(0,7).map(x=><article key={x.id}><div><b>{x.event_type.replaceAll('_',' ')}</b><small>{x.status} • severity {x.severity}</small></div><em>{x.assignee||'Unassigned'}</em></article>)}</div></div>
